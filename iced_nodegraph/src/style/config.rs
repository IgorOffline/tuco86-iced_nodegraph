...
use super::{EdgeBorder, EdgeCurve, EdgeShadow, NodeBorder, NodeConfig, NodeShadow, PinShape};

// `NodeConfig` (and its `merge`/`has_overrides`/`resolve`/`From<NodeStyle>`) is
// generated by `style_config!` next to `NodeStyle`; only the ergonomic setters
// live here.
impl NodeConfig {
    pub fn new() -> Self {
        Self::default()
...
/// Lift a resolved shadow into its config form (used by the generated
/// `From<NodeStyle> for NodeConfig`).
impl From<NodeShadow> for ShadowConfig {
    fn from(s: NodeShadow) -> Self {
        Self {
